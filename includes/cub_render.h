...
void		start_hooks_in_window(t_win *win, t_win_hooks win_hooks,
				void *data);
...
int			draw_player(t_cub *cub, t_map_editor map_editor, t_color col,
				t_img *img);
void		draw_sky_and_ground(t_cub *cub, int wall_top, size_t wall_n,
				int wall_bottom);
...
// REMAP
t_point		remap_point(t_point pt, int zoom, t_point center,
				t_resolution res);
t_point		undo_remap_point(t_point pt, int zoom, t_point center,
				t_resolution res);
...
// MATH
int			calculate_deltas(t_player *player, t_point *deltas, t_point *fov1,
				t_point *fov2);
int			update_player_angle(t_player *player, t_point *deltas,
				t_angle *angle);
...
t_point		cmp_dists(t_point start, t_point *dist1, t_point *dist2,
				t_point *dist3);
